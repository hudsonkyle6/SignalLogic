...
def compute_readiness_score() -> ReadinessReport:
    ...
    po_i = pred_res.get("predicted_value", 0.65) * {
        ...
        "Converging": 0.85,
        "Drifting": 0.55,
    }.get(pred_res.get("predicted_state", "Drifting"), 0.55)
...
def run_lookout_daily(dry_run: bool = False):
    ...
    recent_scar_count = sum(
        1
        for e in psr_events[-10:]
        if e.get("event") in ("INTERVAL_FAILED", "GATE_DENIED")
    )
    events_typed = [
        PSREvent.from_dict(e) for e in psr_events
    ]  # assuming from_dict helper exists
    trust = compute_trust(events_typed)
    scope_multiplier = scar_conditioning(trust, recent_scar_count)

    print(
        f"  Endurance cycles: {endurance_cycles} | Trust: {trust:.3f} | Scope mult: {scope_multiplier:.3f}"
    )
...
if __name__ == "__main__":
    run_lookout_daily(
        dry_run=True
    )  # change to False when ready for live gate + execution
