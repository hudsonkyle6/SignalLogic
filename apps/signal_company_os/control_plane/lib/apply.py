...
def run_apply(policy: Policy, plan_path: Path, mode: str, root_override: Optional[str] = None) -> int:
    ...
    if str(plan.root) != str(apply_root):
        raise SystemExit(f"Refusing apply: plan.root={plan.root} != apply_root={apply_root}")

    if policy.require_gate_for_apply:
        print("\n[GATE WARDEN] → Human required")
        print(f"  Purpose : Apply plan")
        ...
        if sig.lower() in ("deny", "n", "no"):
            ...
            return 2
        append_gate(policy.gate_ledger_path, purpose="Apply plan", scope=f"{plan_path} ops={len(plan.ops)}", signature=sig)

    logs = apply_ops(plan_root=apply_root, repo_root=repo_root, never_touch=policy.never_touch, ops=plan.ops)
    for ln in logs:
        print(ln)
