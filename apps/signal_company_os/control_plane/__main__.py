...
def main() -> int:
    ...
    parser.add_argument(
        "--policy", default="apps/signal_company_os/control_plane/policy.yaml"
    )
    ...
    parser.add_argument(
        "--scope", default="slp+git", help="propose scope: slp, git, slp+git"
    )
    ...
    if args.cmd == "observe":
        return run_observe(
            policy, out_dir=Path(args.out), mode=args.mode, root_override=args.root
        )
    if args.cmd == "propose":
        return run_propose(
            policy,
    ...
    if args.cmd == "apply":
        ...
        return run_apply(
            policy, plan_path=Path(args.plan), mode=args.mode, root_override=args.root
        )
