...
pytestmark = pytest.mark.skipif(not _DEPS_AVAILABLE, reason="pandas/yaml not installed")
...
def _make_stability_yaml(
    tmp_path: Path,
    *,
    enabled: bool = True,
    mode: str = "assist_only",
    escalation: bool = False,
    override_human: bool = False,
) -> Path:
    contract = {
        "StabilityContract": {
...
class TestEnforceAUD:
    def test_valid_contract_passes(self, tmp_path):
        from intelligence.oracle.validate import enforce_assist_under_discipline
...
class TestPhysicalTruthNotOverwritten:
    ...
    def test_physical_truth_change_raises(self):
        ...
        df_before = _valid_df()
        df_after = _valid_df()
        df_after.loc[df_after.index[-1], "Amplitude"] = (
            0.99  # Amplitude is physical truth
        )
...
class TestValidateOracleOutputIntegrity:
    ...
    def test_physical_truth_modified_raises(self):
        from intelligence.oracle.validate import (
            validate_oracle_output_integrity,
            OracleContractError,
        )
