...
from rhythm_os.core.dark_field.store import (
    append_wave_from_hydro,
    _daily_file,
    PENSTOCK_DIR,
)
...
class TestAppendWaveFromHydro:
    def test_creates_file(self, tmp_path, monkeypatch):
        monkeypatch.setattr("rhythm_os.core.dark_field.store.PENSTOCK_DIR", tmp_path)
        w = _wave()
    ...
    def test_creates_parent_dirs_lazily(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("rhythm_os.core.dark_field.store.PENSTOCK_DIR", nested)
        w = _wave()
        path = append_wave_from_hydro(w, anchor_date=FIXED_DATE)
        assert path.exists()
