...
def _bar(
    value: float,
    width: int = 12,
    style_full: str = "green",
    style_empty: str = "grey50",
) -> "Text":
...
def _panel_system(readiness: ReadinessStatus) -> Panel:
    ...
    proc_rec = next(
        (r for r in reversed(meter_records) if r.get("lane") == "proc"), None
    )
    net_rec = next(
        (
            r
            for r in reversed(meter_records)
    ...
    if proc_rec:
        ...
        if cpu_rate is not None:
            bar = _bar(
                min(float(cpu_rate), 1.0), style_full="yellow", style_empty="grey23"
            )
            t.add_row(
                "CPU",
                bar,
                Text(f"{float(cpu_rate) * 100:.1f}% core-eq", style="yellow"),
            )
        if rss is not None:
            rss_mb = float(rss) / 1_048_576
            bar = _bar(
                min(rss_mb / 512.0, 1.0), style_full="yellow", style_empty="grey23"
            )
    ...
    if cpu_rec:
        ...
        if mhz is not None:
            t.add_row(
                "CPU MHz", Text(f"{float(mhz):.0f} MHz", style="dim yellow"), Text("")
            )
    ...
    t.add_row(
        "Records",
        Text(str(count), style="bold yellow"),
...
def _panel_natural(readiness: ReadinessStatus) -> Panel:
    ...
    pressure_rec = next(
        (r for r in reversed(nat_records) if r.get("channel") == "helix_projection"),
        None,
    )
    thermal_rec = next(
        (r for r in reversed(nat_records) if r.get("channel") == "thermal"), None
    )
    ...
    t.add_row(
        "Location",
        Text(f"{label}", style="bold green"),
        Text(f"({lat:.4f}°N, {abs(lon):.4f}°{'W' if lon < 0 else 'E'})", style="dim"),
    )
    ...
    if pressure_rec:
        ...
        t.add_row(
            "Pressure", bar, Text(f"{label_txt}  {phase_txt}  {coh_txt}", style="green")
        )
    else:
    ...
    t.add_row(
        "Records",
        Text(str(readiness.natural_count), style="bold green"),
        _ready_badge(readiness.natural_ready),
    )
...
def _panel_domain() -> Panel:
    ...
    for (
        ...
        if rec:
            ...
            field_str = (
                field_fmt.format(float(field_val)) if field_val is not None else "—"
            )
...
def _panel_cycle(cycle_result: Optional[Any] = None) -> Panel:
    ...
    if cycle_result is None:
        ...
        ts = datetime.fromtimestamp(cycle_result.cycle_ts, tz=timezone.utc).strftime(
            "%H:%M:%S UTC"
        )
        ...
        t.add_row(
            "Rejected",
            Text(
                str(cycle_result.rejected),
                style="red" if cycle_result.rejected else "dim",
            ),
        )
        ...
        t.add_row(
            "Quarantined",
            Text(
                str(cycle_result.spillway_quarantined),
                style="red" if cycle_result.spillway_quarantined else "dim",
            ),
        )
        ...
        if cs:
            ...
            t.add_row(
                "Convergence", Text(f"{ev} events ({strong} strong)", style="magenta")
            )
            for event in cs.get("convergence_events", []):
                ...
                line.append(
                    "● " if is_strong else "○ ",
                    style="bold magenta" if is_strong else "dim magenta",
                )
                line.append(
                    " + ".join(domains), style="magenta" if is_strong else "dim"
                )
...
def _build_display(
    ...
    helix_panel = Panel(
        helix_text, title="[bold magenta]HELIX[/]", border_style="magenta", width=30
    )
    ...
    with Live(console=console, refresh_per_second=4, screen=True) as live:
        while True:
            live.update(
                _build_display(
                    rotation=rotation,
                    cycle_result=cycle_result or _load_last_cycle_result(),
                )
            )
            time.sleep(interval)
...
def _fallback_print() -> None:
    ...
    print(
        f"TIER II  NATURAL: {readiness.natural_count} records  {'[WARM]' if readiness.natural_ready else '[COLD]'}"
    )
...
def main() -> None:
    ...
    ap.add_argument(
        "--animate", action="store_true", help="continuously rotating helix animation"
    )
    ap.add_argument(
        "--interval",
        type=float,
        default=5.0,
        help="refresh interval for --watch (default: 5s)",
    )
    ap.add_argument(
        "--fps",
        type=float,
        default=8.0,
        help="frames per second for --animate (default: 8)",
    )
    args = ap.parse_args()
