...
class HydroPacket:
    ...
    # ─────────────────────────────
    seasonal_band: Optional[str] = (
        None  # "winter" | "spring_transition" | "summer" | "fall_transition"
    )
    pattern_confidence: Optional[float] = (
        None  # [0, 1] — stability of current seasonal pattern
    )
    forest_proximity: Optional[float] = (
        None  # [0, 1] — 0 = pasture center, 1 = forest edge
    )
